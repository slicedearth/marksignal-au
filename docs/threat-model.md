...
| Required source schema drift | Required and unique-column checks, archive member checks, strict selected models, schema fingerprint for additive-column review, fail-closed publication |
...
| Client navigation injection or resource exhaustion | Exact same-origin signal, applicant-history, and event-history routes, record-key checks, redirect rejection, HTML content checks, two-megabyte response cap, page validation, script removal, and normal-link fallback |
| Client dataset injection or resource exhaustion | Exact same-origin signal JSON path, redirect and content-type rejection, ten-megabyte response cap, 20,000-record cap, strict field validation, text-node rendering, allowlisted local links, and 50-card client pages |
